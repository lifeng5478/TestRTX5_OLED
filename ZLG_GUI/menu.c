...
{  WINDOWS  *mwin;
   ...
   {  if( (xx+MMENU_WIDTH) > (mwin->x + mwin->with) ) return(0);
      ...
      GUI_PutNoStr(xx+2, mwin->y + 14, men->str[i], MMENU_TEXT_LEN);    // 书写主菜单文字，最后一个参数表示文字长度，ZXE修改
      xx += MMENU_WIDTH;
...
{  WINDOWS  *mwin;
   ...
   {  if( i == (men->state) )								// 判断所选择的菜单项
      {  /* 显示背景色 */
         ...
         /* 显示菜单字符 */
         GUI_ExchangeColor();
         GUI_PutNoStr(xx+1, yy+1, men->str[i], SMENU_TEXT_LEN); // 书写子菜单文字，最后一个参数表示文字长度，ZXE修改
         GUI_ExchangeColor();
      }
      else
      {  /* 显示背景色 */
         ...
         /* 显示菜单字符 */
         GUI_PutNoStr(xx+1, yy+1, men->str[i], SMENU_TEXT_LEN); // 取消书写子菜单文字，最后一个参数表示文字长度，ZXE修改
      }   
...
{  WINDOWS  *mwin;
   ...
   yy += old_no*11;
   GUI_RectangleFill(xx, yy, xx+SMENU_WIDTH-2, yy+10, back_color);
   GUI_PutNoStr(xx+1, yy+1, men->str[old_no], SMENU_TEXT_LEN);  // 取消书写子菜单文字，最后一个参数表示文字长度，ZXE修改
   /* 选择新的子菜单 */
   yy = mwin->y + 22 + 2;
   yy += new_no*11;
   GUI_RectangleFill(xx, yy, xx+SMENU_WIDTH-2, yy+10, disp_color);
   GUI_ExchangeColor();
   GUI_PutNoStr(xx+1, yy+1, men->str[new_no], SMENU_TEXT_LEN);  // 书写子菜单文字，最后一个参数表示文字长度，ZXE修改
   GUI_ExchangeColor();
}
...
//   if( (ico->state)==0 )
//   { 
//		 GUI_LoadPic(ico->x-5, ico->y+33, (uint8 *) ico->title+24, 6, 12);
//		 GUI_LoadPic(ico->x-5+6, ico->y+33, (uint8 *) ico->title, 12, 12);
//		 GUI_LoadPic(ico->x+7+6, ico->y+33, (uint8 *) ico->title+24, 12, 12);
//		  GUI_LoadPic(ico->x+12+6, ico->y+33, (uint8 *) ico->title+48, 12, 12);
//		 	 GUI_LoadPic(ico->x+24+6, ico->y+33, (uint8 *) ico->title+24, 6, 12);
//		   GUI_LoadPic(ico->x, ico->y, (uint8 *) ico->icodat, 32, 32);			// 显示ICO图
//   }
//   else
//   {  GUI_LoadPic1(ico->x-5, ico->y+33, (uint8 *) ico->title+24, 6, 12);
//		 GUI_LoadPic1(ico->x-5+6, ico->y+33, (uint8 *) ico->title, 12, 12);
//		 GUI_LoadPic1(ico->x+7+6, ico->y+33, (uint8 *) ico->title+24, 12, 12);
//		  GUI_LoadPic1(ico->x+12+6, ico->y+33, (uint8 *) ico->title+48, 12, 12);
//		 	 GUI_LoadPic1(ico->x+24+6, ico->y+33, (uint8 *) ico->title+24, 6, 12);
//		   GUI_LoadPic1(ico->x, ico->y, (uint8 *) ico->icodat, 32, 32);			// 显示ICO图
//   }
   return(1);
}
#endif
